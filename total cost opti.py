...
PARCEL_ALLOCATIONS_PARQUET = 'all_days_parcel_allocations_optimized.parquet'
OUTPUT_TOTAL_COSTS_CSV = 'total_yearly_costs_per_sp_optimized.csv'
OUTPUT_COST_BREAKDOWN_PLOT = 'sp_cost_breakdown_plot_optimized.png'
...
# --- 2.1 Load Optimized Service Point Configuration ---
print("\nStep 2.1: Loading optimized service point configuration")
try:
    optimized_sp_df = pd.read_csv(OPTIMIZED_SP_CSV)
    optimized_sp_df['sp_id_int'] = optimized_sp_df['sp_id_int'].astype(str)
    optimized_sp_df.set_index('sp_id_int', inplace=True)
    print(f"  Loaded optimized service point config for {len(optimized_sp_df)} service points.")
except FileNotFoundError:
    print(f"ERROR: Optimized service point configuration file not found at '{OPTIMIZED_SP_CSV}'.")
    exit()
except Exception as e:
    print(f"ERROR: Could not read '{OPTIMIZED_SP_CSV}': {e}")
    exit()

# print("  Filtering parcel allocations to include only opened service points from optimized config")
# parcel_allocations_df['sp_id'] = parcel_allocations_df['sp_id'].astype(str)
# open_sp_ids = optimized_sp_df[optimized_sp_df['opened'] == True].index.tolist()
# parcel_allocations_df = parcel_allocations_df[parcel_allocations_df['sp_id'].isin(open_sp_ids)]
parcel_allocations_df['sp_id'] = parcel_allocations_df['sp_id'].astype(str)
print(f"  Parcel allocations retained for cost calculation: {len(parcel_allocations_df)} rows (no SP filter applied).")
...
print("  Overriding fixed costs using optimized configuration")
fixed_costs_df = optimized_sp_df[['capacity']].copy()
fixed_costs_df['fixed_yearly_cost'] = BASE_FIXED_COST_SP  # Apply base cost
fixed_costs_df.index.name = 'service_point_id'
...
if valid_allocations.empty:
    ...
    # Sum parcel‑level costs directly; parquet already holds a full 365‑day period
    total_variable_cost_sim_period_per_sp = (
        valid_allocations.groupby('sp_id')['parcel_delivery_cost'].sum()
    )

    variable_yearly_cost_per_sp = total_variable_cost_sim_period_per_sp.copy()

    variable_costs_per_sp_df = variable_yearly_cost_per_sp.reset_index()
    variable_costs_per_sp_df.rename(
        columns={'parcel_delivery_cost': 'variable_yearly_cost', 'sp_id': 'service_point_id'},
        inplace=True
    )
...
# Filter parcel allocations to only opened service points using optimized configuration
open_sp_ids = optimized_sp_df[optimized_sp_df['opened'] == True].index.tolist()
parcel_allocations_filtered = parcel_allocations_df[parcel_allocations_df['sp_id'].isin(open_sp_ids)].copy()

if parcel_allocations_filtered.empty or 'sp_id' not in parcel_allocations_filtered.columns or 'day' not in parcel_allocations_filtered.columns:
    ...
    daily_parcels_per_sp = parcel_allocations_filtered.groupby(['day', 'sp_id']).size().reset_index(name='num_daily_parcels')
