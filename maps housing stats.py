...
if "x_rd" not in cbs_df.columns and "x" in cbs_df.columns:
    cbs_df.rename(columns={"x": "x_rd"}, inplace=True)
if "y_rd" not in cbs_df.columns and "y" in cbs_df.columns:
...
# Rename housing stats columns for clarity
cbs_web = cbs_web.rename(columns={
    "houses": "houses",
    "home.ownership..": "home_ownership_pct",
    "rental..": "rental_pct",
    "social.housing..": "social_housing_pct",
    "vacant.houses": "vacant_houses"
})
...
# List of housing-statistics to map
stats = [
    "houses",
    "home_ownership_pct",
    "rental_pct",
...
for stat in stats:
    ...
    # Plot CBS squares by this statistic
    cbs_web.plot(
        ax=ax,
        column=stat,
        cmap="viridis",
    ...
    # Add colorbar
    vmin, vmax = cbs_web[stat].min(), cbs_web[stat].max()
    ...
    cbar.set_label(stat.replace("_", " ").title(), fontsize=9)
    ...
    # Draw the basemap under all layers
    cx.add_basemap(ax, crs=3857, source=cx.providers.CartoDB.Positron, zorder=-3)

    ax.set_axis_off()
    plt.tight_layout()
    # Save each map with a filename indicating the statistic
    plt.savefig(f"{stat}_map.png", dpi=300)
    print(f"Saved {stat} map to {stat}_map.png")