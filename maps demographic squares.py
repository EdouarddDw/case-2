...
# Draw CBS 500 m squares coloured by population density
pop_column = "population" if "population" in cbs_web.columns else None
cbs_web.plot(
    ax=ax,
    column=pop_column,
    cmap="viridis",
...
# Add a colour bar legend for population
if pop_column:
    import matplotlib as mpl
    vmin, vmax = cbs_web[pop_column].min(), cbs_web[pop_column].max()
    sm = mpl.cm.ScalarMappable(cmap="viridis", norm=mpl.colors.Normalize(vmin=vmin, vmax=vmax))
    sm._A = []  # hack: required for matplotlib < 3.6
    cbar = fig.colorbar(sm, ax=ax, shrink=0.65, pad=0.01)
    cbar.set_label("Population per CBS 500 m square", fontsize=9)
...
ax.set_axis_off()
plt.tight_layout()
plt.savefig("population_map.png", dpi=300)
print("Saved map with all nodes to population_map_map.png")