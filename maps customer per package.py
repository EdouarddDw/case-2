...
# attach customer‑per‑pickup ratio
node_highlight = node_highlight.merge(
    sp_df[["sp_id", "cust_per_pickup"]],
    left_on="node_id",
    right_on="sp_id",
    how="left",
)
...
ratio_min = node_highlight["cust_per_pickup"].min()
ratio_max = node_highlight["cust_per_pickup"].max()
node_highlight["dot_size"] = size_min + (
    (node_highlight["cust_per_pickup"] - ratio_min) / (ratio_max - ratio_min)
) * (size_max - size_min)
...
# add text labels with the customer‑per‑pickup value
for row in node_highlight.itertuples():
    if pd.notna(row.cust_per_pickup):
        ax.text(
            row.geometry.x,
            row.geometry.y,
            f"{row.cust_per_pickup:.1f}",
            fontsize=8,
            ha="center",
            va="center",
            color="black",
...
ax.set_axis_off()
plt.tight_layout()
plt.savefig("customer_per_package_map.png", dpi=300)
print("Saved map with all nodes to all_nodes_map.png")